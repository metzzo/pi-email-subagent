...
- \`high\`: blockers, corrections, or discoveries that should affect ongoing work. The broker attempts presentation through Pi's high-priority boundary. A correlated high reply is not hidden by a slow sibling in an active multi-request collector: ordinary presentation wins, that wait ends partial, and the already-presented body is omitted there.
- \`low\`: ordinary delegation, completed results, and non-urgent information. The broker queues it until the recipient finishes current work. For main specifically, low mail that arrives while main is busy remains in the bounded durable broker queue instead of being handed to Pi as a follow-up; a collector can claim it, otherwise a one-shot macrotask presents it after \`agent_settled\` if the same session is still idle. A backlog may drain one message per settlement. Low mail arriving while main is idle is presented promptly. Main queue count/bytes aggregate every current and historical main alias; high mail is exempt.
