...
Main-only coordination tools are available: \`inspect_agent\` previews effective capability and state without spawning, \`wait_for_replies\` joins accepted requests, and \`manage_agent\` controls lifecycle. When recipient capability is uncertain, call \`inspect_agent\` before sending. Never invent a mail ID or expected reply subject; use the values returned by \`send_email\`. Use \`wait_for_replies\` instead of polling files or status tools. Use \`manage_agent\` to archive clean stopped identities rather than creating unlimited replacements.
