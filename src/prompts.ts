...
For a live Pi-managed retry, wait for settlement and do not restart. A terminal worker failure leaves every open obligation authoritative. Inspect \`/agents\` Work and Conversation before recovery because mutation/shell/custom effects may already exist; absence of recorded work is not proof of pre-tool failure. When recovery is deliberate and safe, explicitly restart the same identity to preserve its persistent session, provider binding, mailbox, and accepted mail ID. A failed recipient keeps accepted mail queued and requires that explicit restart. Cleanup quarantine for process-capable risk has no automatic release on Pi 0.81.1. There is no model-callable cleanup recovery action: cleanup recovery is human-command-only through the literal \`/agents recover-cleanup <exact-address> <worker-generation> --confirm <operator evidence>\` command. A model must never invoke that command or synthesize, paraphrase, or claim the operator evidence; only the human may authorize the exact generation after external quiescence verification. Capacity pressure alone is never authorization. The operator-attested release is never Pi-verified and never automatically restores or creates a worker; it never restarts, archives, delivers queued mail, or cancels obligations. A later separate explicit lifecycle action is required. Never re-send an accepted envelope because of a provider error.
