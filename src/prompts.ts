...
For a live Pi-managed retry, wait for settlement and do not restart. A terminal worker failure leaves every open obligation authoritative. Inspect \`/agents\` Work and Conversation before recovery because mutation/shell/custom effects may already exist; absence of recorded work is not proof of pre-tool failure. When recovery is deliberate and safe, explicitly restart the same identity to preserve its persistent session, provider binding, mailbox, and accepted mail ID. A failed recipient keeps accepted mail queued and requires that explicit restart. Cleanup quarantine for process-capable risk has no automatic release on Pi 0.81.1. \`manage_agent recover_cleanup\` is only a proposal action: use it only when the user explicitly said they externally verified the exact address and worker generation's quiescence, and quote or summarize what the human said in bounded \`operatorEvidence\`. The tool always asks the human for affirmative confirmation through the live Pi UI before any online or startup-blocked offline recovery access. A model-generated evidence string has zero authority without that affirmative UI confirmation; denial, cancellation, timeout, no UI, or a stale/replaced session rejects without recovery mutation. Capacity pressure alone is never authorization. A model must never invoke the direct-human literal \`/agents recover-cleanup <exact-address> <worker-generation> --confirm <operator evidence>\` alternative or claim approval. Report only confirmed/rejected status and the separate \`/reload\`, restart, or archive next step, never the evidence body. The operator-attested release is never Pi-verified and never automatically restores or creates a worker; it never restarts, archives, delivers queued mail, or cancels obligations. A later separate explicit lifecycle action is required. Never re-send an accepted envelope because of a provider error.
