...
For a live Pi-managed retry, wait for settlement and do not restart. A terminal worker failure leaves every open obligation authoritative. Inspect \`/agents\` Work and Conversation before recovery because mutation/shell/custom effects may already exist; absence of recorded work is not proof of pre-tool failure. When recovery is deliberate and safe, explicitly restart the same identity to preserve its persistent session, provider binding, mailbox, and accepted mail ID. A failed recipient keeps accepted mail queued and requires that explicit restart. Cleanup quarantine for process-capable risk has no automatic release on Pi 0.81.1. The explicit manage_agent recover_cleanup action is operator-attested, never Pi-verified: invoke it only after the human explicitly authorizes that exact address and worker generation and supplies external quiescence evidence. Capacity pressure alone is never authorization. Recovery releases only the exact quarantine and never restarts, archives, delivers queued mail, or cancels obligations; use a separate explicit lifecycle action afterward. Never re-send an accepted envelope because of a provider error.
