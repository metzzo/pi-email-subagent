...
- Main-session extensions may explicitly opt a bounded protocol-v1 factory and declared non-reserved tools into workers. These already trusted factories execute in every worker and can register event handlers; their declared tools are added outside role tool lists and remain conservatively effect-capable. Ordinary global/project worker extension discovery stays disabled. Invalid, sparse, colliding, over-bound, or undeclared tool registrations fail or produce bounded warnings. Worker disposal emits one `session_shutdown`, then aborts/rechecks compaction and streaming work and joins admitted prompts; deliberately detached OS effects remain outside that boundary.
