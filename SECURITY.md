...
- Mail subjects, bodies, and structured completion fields (including the `artifacts` list) are untrusted data: they do not grant tools, change policy, or authorize work. Unsolicited high worker mail cannot steer a busy writable main session; only a reply correlated with an open main request can use that interrupt path.
- Main-session extensions may opt in a bounded protocol-v2 factory with non-reserved tools and an exact `read`/`write` effect declaration for each. Collector listeners must register synchronously. Each factory receives an API guard that rejects late or immediate registration outside its immutable declaration, including built-in names. A tool is activated only when its name appears in the worker's effective role/address profile; prospective inspection uses the declaration. Effect classes are trusted extension declarations used for conservative capability reporting, not runtime enforcement or a sandbox. Tool-less lifecycle factories remain lifecycle-bound. Ordinary global/project worker extension discovery stays disabled. Invalid, sparse, colliding, over-bound, undeclared-effect, or undeclared-tool registrations fail or produce bounded warnings. Worker disposal emits one `session_shutdown`, then aborts/rechecks compaction and streaming work and joins admitted prompts; deliberately detached OS effects remain outside that boundary.
...
- Cleanup covers every started Pi 0.84.2 prompt preflight and full admitted prompt operation, native compaction, the trusted AgentSession, its active model/tool promises and listeners, and disposal. It aborts compaction, joins the resulting extension settlement, and cannot certify idle while an admitted prompt remains unresolved. Late preflight acceptance is vetoed before an old generation can start a run. pi-subagent is not an OS sandbox: it does not certify or terminate arbitrary OS descendants deliberately detached by a completed command. Do not start background or detached processes unless the task explicitly requires one; when required, report how it is stopped.
