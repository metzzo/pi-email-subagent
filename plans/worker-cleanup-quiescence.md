...
The extension containment layer is implemented. Every broker teardown route now joins one cleanup lease bound to the exact `WorkerTransport` and monotonically assigned worker generation. Starting cleanup synchronously removes routing and subscriptions but retains the address quarantine, activation lease, and any active/concurrency slot. Caller timeout marks bounded persisted `pending`/`unknown` cleanup phases without cancelling or forgetting the underlying Promise. Late verified receipts release only their exact lease; after a caller-visible restart failure they leave the identity paused and never create a hidden replacement. Late rejection stays quarantined. Restart, archive, clear-failure, exact-address restoration, high-priority steering, and new mutable scheduling fail closed while unknown. Accepted mail is materialized and registered as a deferred admission with its stable journal ID, then the ordinary pump delivers it after the last verified release. Persisted `pending` state becomes sticky `unknown` after process restart. Shutdown and session handoff retain namespace ownership rather than starting a replacement over unknown cleanup.

`SdkWorker.cleanup()` is one reused Promise. It suppresses events and unsubscribes immediately, remains pending until the actual SDK abort settles, invokes session disposal once, and returns content-free provider/session/tool confidence. Broker waits—not this authoritative operation—bound caller responsiveness. A clean SDK session can verify callback settlement only if no process-capable tool ran in the generation. Once Bash starts, process risk remains generation-sticky even after the call reports success and leaves the active map; arguments, commands, output, and environment never enter the report or registry.

The directly recorded representative Linux tests use the real Pi 0.81.1 SDK worker and built-in Bash. One starts a Node parent plus an ordinary same-process-group Node descendant; on stop, both exact process identities become absent and the heartbeat stops. Another makes Bash return success while a redirected background heartbeat remains alive; stop and restart stay quarantined without overlap. A real broker `SIGKILL` during the first topology is recovered after exact-owner fencing as sticky abandoned-owner cleanup before writable restoration. Red and green cleanup kills only the recorded PID plus `/proc` start-time identity. These cases do not reproduce the historical surviving `worker.js` launch pattern and do not prove escaped descendants, other topologies, or other platforms.
