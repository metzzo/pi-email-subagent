...
- Missing, stale, or dead namespace owner sidecars/lock directories remain fail-closed orphans. Automatic abandoned-owner takeover is disabled because Pi 0.81.1 cannot atomically bind lock acquisition to owner publication; the supported manual recovery is the exact audited action below.
- Added explicit exact-generation cleanup recovery and tightened it after correctness/Linux review. `manage_agent recover_cleanup` is now a model-callable proposal only after explicit human exact-generation external verification; it always requests a one-use live Pi UI confirmation bound to the canonical address, generation, sanitized evidence, tool call, and session generation before any recovery access. Model text alone has zero authority, and denial/cancellation/timeout/no UI/stale context fail closed. The literal `/agents recover-cleanup … --confirm …` remains the direct human-command-only alternative. Both paths share the same online/startup-blocked callback, reject live/SIGSTOPed owners and all live/pending worker facts, force inactive failed `operator-attested`/`operator-released` state that is never Pi-verified, never auto-restores/delivers on reload, and reject stale audits after a later epoch activates. Offline recovery applies the broker's shared abandoned-owner transition across every namespace record before removing the orphan; its narrow upgrade migration canonicalizes only the prior exact same-generation paused/operator-released/no-hold shape to failed so an idempotent crash retry can finish. A pre-existing fixed recovery guard is never automatically reclaimed and requires deliberate manual removal only after exact dead-guard/crash-stage inspection. Capacity pressure alone is never authorization; recovery never restarts, archives, reloads, retries providers, or cancels obligations.
...
- Prompt, tool, broker, UI, and operator guidance now share one failure contract: live Pi-managed retries must settle; terminal obligations remain open; possible effects require Work/Conversation review and same-identity/session/provider recovery; same-scope redelegation stays forbidden while the original obligation is open; failed mail queues for restart; unknown process-capable cleanup has no automatic release; every fetched response-required email must be answered; and legacy parents remain responsible while recovered child dependencies are open.
