...
- Low-priority mail to busy main now remains broker-queued instead of being pre-owned by a Pi `followUp`. A later `wait_for_replies(collect:true)` can atomically claim a correlated queued reply; otherwise unclaimed mail flushes at `agent_settled`, while idle delivery and high-priority steering remain prompt. Pi 0.84.2 still has no durable `sendMessage` append acknowledgement.
