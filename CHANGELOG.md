...
- Added explicit exact-generation cleanup recovery: main-only `manage_agent recover_cleanup` and startup-blocked `/agents recover-cleanup … --confirm …` require human authorization plus bounded external-quiescence evidence, reject live/SIGSTOPed owners and all live/pending worker facts, preserve mail/session/other records, and persist an idempotent `operator-attested`/`operator-released` audit that is never Pi-verified. Capacity pressure alone is never authorization; recovery never restarts, archives, reloads, delivers mail, retries providers, or cancels obligations.
