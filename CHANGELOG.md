...
- Added explicit exact-generation cleanup recovery and tightened it after correctness/Linux review: it is human-command-only through `/agents recover-cleanup … --confirm …` and absent from the model-callable `manage_agent` schema. It requires exact human authorization plus bounded external-quiescence evidence, rejects live/SIGSTOPed owners and all live/pending worker facts, forces inactive failed `operator-attested`/`operator-released` state that is never Pi-verified, never auto-restores/delivers on reload, and rejects stale audits after a later epoch activates. Offline recovery applies the broker's shared abandoned-owner transition across every namespace record before removing the orphan; its narrow upgrade migration canonicalizes only the prior exact same-generation paused/operator-released/no-hold shape to failed so an idempotent crash retry can finish. A pre-existing fixed recovery guard is never automatically reclaimed and requires deliberate manual removal only after exact dead-guard/crash-stage inspection. Capacity pressure alone is never authorization; recovery never restarts, archives, reloads, retries providers, or cancels obligations.
