...
const poll = setTimeout(async () => { try { const stack=[root]; while(stack.length){ const d=stack.pop()!; for(const e of await readdir(d,{withFileTypes:true})){ const p=join(d,e.name); if(e.isDirectory()) stack.push(p); else if(e.name === 'mail.jsonl') { const text=await readFile(p,'utf8'); for(const line of text.split('\n')) { try { const ev=JSON.parse(line); const em=ev.email; if(em && em.from===worker && em.to===main && em.kind==='notification' && em.requiresResponse===false && em.inReplyTo===undefined) completionObserved=true; } catch {} } } } } if(completionObserved) child.stdin.end(); } catch {} }, 2_000);
const code = await new Promise<number|null>(resolveCode => { const timer=setTimeout(()=>{ child.kill("SIGTERM"); setTimeout(()=>child.kill("SIGKILL"), 5_000); }, timeout); child.once("close", c=>{clearTimeout(timer);clearTimeout(orderly);clearTimeout(poll);resolveCode(c)}); });
