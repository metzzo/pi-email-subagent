...
const UNSAFE_NATIVE_EXTENSION = resolve(
  "test/e2e/helpers/unsafe-native-provider-extension.ts",
);
...
for (const modelHeaders of [false, true])
  it(
    `real Pi rejects unsafe native ${modelHeaders ? "model headers" : "public provider"} before email acceptance`,
    { timeout: 180_000 },
    async () => {
      ...
      try {
        ...
        const sessionId = (state.data as { sessionId?: string } | undefined)
          ?.sessionId;
        assert.ok(sessionId);

        const mark = client.mark();
        await client.prompt("E2E NATIVE PROVIDER REJECT NOWAIT");
        const send = await client.waitFor(
          sendEnd,
          "unsafe native provider rejection",
          90_000,
          mark,
        );
        ...
        const registry = JSON.parse(
          await readFile(join(stateDir, "registry.json"), "utf8"),
        ) as { agents: unknown[] };
        assert.deepEqual(registry.agents, []);
        const journal = await readFile(
          join(stateDir, "mail.jsonl"),
          "utf8",
        ...
        assert.equal(
          journal.trim(),
          "",
          "native provider preflight rejection journals no email event",
        );
      } finally {
