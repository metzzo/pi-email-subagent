...
it("renders derived capacity, lease, obligations, archive eligibility, and safe recovery", async () => {
  ...
  assert.match(quarantineText, /Pi session\/tool cleanup settlement is unknown.*exact address/is);
  assert.match(quarantineText, /wait for.*live cleanup operation.*restart\/archive.*blocked only for this identity/is);
  assert.doesNotMatch(quarantineText, /recover_cleanup|recover-cleanup|operator-attested/i);
});
