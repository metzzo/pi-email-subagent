...
function pack(overrides: Partial<PackResult> = {}): PackResult {
  return { filename: "pi-email-subagent-0.1.0.tgz", entryCount: PACKAGE_MAX_ENTRIES,
...
describe("authoritative npm package surface policy", () => {
  it("accepts the exact inventory and all size bounds", () => {
    assert.equal(PACKAGE_PATHS.length, PACKAGE_MAX_ENTRIES);
    assert.doesNotThrow(() => assertPackageSurface(pack()));
    assert.doesNotThrow(() => assertPackageSurface(pack({ size: 219906, unpackedSize: 807752 })));
    assert.doesNotThrow(() => assertPackageSurface(pack({ size: 220060, unpackedSize: 807752 })));
  });
  ...
  it("rejects every forbidden internal path and unreviewed src/docs substitutions", () => {
    ...
    for (const path of ["src/surprise.ts", "docs/surprise.md", PACKAGE_PATHS[0]]) {
      assert.throws(() => assertPackageSurface(pack({ files: [...pack().files.slice(0, -1), { path }] })), /inventory differs/);
    }
    assert.throws(() => assertPackageSurface(pack({ files: pack().files.slice(1), entryCount: PACKAGE_MAX_ENTRIES - 1 })), /missing CHANGELOG/);
  });
