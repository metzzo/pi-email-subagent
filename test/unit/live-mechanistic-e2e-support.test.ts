...
for(const [name,mutate,reason] of [["reply",(b:any,i:any)=>{i.events=i.events.map((e:any)=>e.email?.id===b.final.id?{...e,email:{...e.email,kind:"reply",inReplyTo:b.invocation.id}}:e)},"graph contains a reply"],["response",(b:any,i:any)=>{i.events=i.events.map((e:any)=>e.email?.id===b.final.id?{...e,email:{...e.email,requiresResponse:true}}:e)},"graph contains response-required mail"],["completion",(b:any,i:any)=>{i.events=i.events.map((e:any)=>e.email?.id===b.final.id?{...e,email:{...e.email,completion:{status:"success"}}}:e)},"graph contains completion metadata"],["signal",(b:any,i:any)=>{i.events=i.events.map((e:any)=>e.type==="job.terminal"?{...e,job:{...e.job,signal:"SIGTERM"}}:e)},"runtime did not succeed cleanly"],["cleanup",(b:any,i:any)=>{i.events=i.events.map((e:any)=>e.type==="job.terminal"?{...e,job:{...e.job,cleanup:{...e.job.cleanup,state:"cleanup-unknown"}}}:e)},"direct-child cleanup is not confirmed"],["outcome missing",(b:any,i:any)=>{i.events=i.events.filter((e:any)=>e.email?.id!==b.outcome.id)},"missing or mismatched automatic outcome"]] as const) it(name,async()=>{const b=await baseline();const i=input(b);mutate(b,i);const r=validateLiveGraph(i);assert.equal(r.ok,false);assert.ok(r.reasons.includes(reason),r.reasons.join("; "));});
