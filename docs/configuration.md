...
`abortTimeoutMs` and `disposeTimeoutMs` bound caller responsiveness, not the lifetime of the underlying cleanup Promise. Missing either deadline leaves cleanup registered and observed. Routing detaches, but capacity/address ownership stays quarantined until a structured verified report; unknown cleanup is persisted and survives restart. A restored `unknown` quarantine has no automatic verification or release. `manage_agent recover_cleanup` is a model proposal only after explicit exact-generation human external verification and always requires affirmative one-use live Pi UI confirmation before recovery access; model text, capacity pressure, and timeout duration never authorize it. The literal `/agents recover-cleanup` remains the direct human-command-only alternative. A confirmed transition records `operator-attested`, not Pi-verified, remains failed, and never automatically restores/creates a worker or delivers mail. The defaults are not process-quiescence guarantees, and increasing them does not create cancellation. See [lifecycle.md](lifecycle.md) for the Pi 0.81.1 active-tool receipt limit.
