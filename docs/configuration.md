...
Per-run turn/tool/token budgets fail the exact worker with a bounded `IDENTITY_*_BUDGET` diagnostic while preserving its mail and possible-effect evidence. Before each assistant provider request (including Pi agent retries), turn admission stops at `maxTurns` and continuation stops once reported input + output consumption reaches `maxTokens`. A final turn completing exactly at either limit is allowed. Usage is only known after a provider response: an admitted response can overshoot the token limit, and its tool calls may finish; this is not a hard provider-spend cap. Provider-internal retries, cache-token accounting, and Pi summarization costs are not included in this assistant input/output limit. A rejected continuation appears as a native zero-usage error in Conversation but does not increment admitted-turn usage. Baselines reset for new delivery/steering and enforcement cycles, not for Pi retries. Consecutive terminal failures persist on the identity; once the circuit opens, another restart is rejected until main stops the inactive identity, reviews effects, and uses `clear_failure`. A clean completed run resets the counter.
...
- `instructions` and `modelPolicy` are each at most 16 KiB of UTF-8 at configuration load. Their combined complete values must also fit the selected model's input allowance, capped at 32 KiB with no synthetic minimum. Prompt additions and mail envelopes share one metadata interpretation: `contextWindow - maxTokens`, or `contextWindow` when `maxTokens >= contextWindow` denotes uncapped output. Invalid metadata has zero allowance. Without provider tokenizers, additions permit at most one UTF-8 byte per available input token; fixed system prompt and history cost remain Pi-owned and outside this additions-only budget. If required additions cannot fit, `PROMPT_ADDITIONS_TOO_LARGE` rejects new worker mail before acceptance and blocks worker startup; policy and role instructions are never shortened or omitted to admit work. Main startup and main sends also require its complete model policy to fit;
