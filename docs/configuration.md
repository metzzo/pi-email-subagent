...
`abortTimeoutMs` and `disposeTimeoutMs` bound caller responsiveness, not the lifetime of the underlying cleanup Promise. Missing either deadline leaves cleanup registered and observed. Routing detaches, but capacity/address ownership stays quarantined until a structured verified report; unknown cleanup is persisted and survives restart. A restored `unknown` quarantine has no automatic verification or release. The separate human-command-only literal `/agents recover-cleanup` path (never a model-callable action) is available only after explicit human authorization and external quiescence evidence; it records `operator-attested`, not Pi-verified, remains failed, never automatically restores/creates a worker or delivers mail, and capacity pressure alone is never authorization. The defaults are not process-quiescence guarantees, and increasing them does not create cancellation. See [lifecycle.md](lifecycle.md) for the Pi 0.81.1 active-tool receipt limit.
