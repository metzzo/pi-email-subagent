...
- `low` (default): queued and delivered in batches when a worker recipient settles. At most `maxBatchMessages` (32) or `maxBatchBytes` (512 KB) per delivery batch, high priority first, FIFO within a priority. For main, low mail received while the agent is busy remains in the durable broker queue instead of being handed to Pi as a `followUp`; `wait_for_replies(collect:true)` may claim a correlated queued reply, otherwise remaining mail flushes at Pi `agent_settled`. Main-idle low mail presents promptly. Individual envelopes and mail-tool results remain within Pi's context-safe 50 KB / 2000-line output recommendation; `fetch_emails` pages independently when a delivery batch is larger. At most `maxConcurrent` (4) agents run at once; the rest wait in a fair queue that prioritizes aged (≥30 s) and high-priority mail.
- `high`: if the recipient is mid-run, the email **steers** it at the next safe agent boundary and is marked delivered immediately. Otherwise it is queued ahead of low mail. Main steering behavior is unchanged by deferred low presentation.
