...
- `/agents recover-cleanup <exact-address> <worker-generation> --confirm <operator evidence>`: human-only online or startup-blocked operator-attested quarantine release; it requires explicit human authorization for that exact generation and is not Pi verification.
