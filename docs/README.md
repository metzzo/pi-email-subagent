...
- `manage_agent recover_cleanup`: model proposal after explicit exact-generation human verification; always opens one-use Pi UI confirmation and reports confirmed/rejected without returning the evidence body.
- `/agents recover-cleanup <exact-address> <worker-generation> --confirm <operator evidence>`: direct human-command-only online or startup-blocked alternative; it is operator-attested, not Pi verification.
