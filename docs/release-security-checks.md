...
`npm test` inside `validate` drives the exact Pi 0.84.2 SDK/RPC fixtures. Evidence assertions parse their canonical formats: JSONL RPC records by event type/tool-call ID, mail journal JSON events by stable mail ID, registry JSON by exact address/generation, Pi session branches by entry/tool-call/message ID, process readiness/heartbeat JSON by schema, and the test runner's TAP result/exit status. Text grep counts are not release evidence.
