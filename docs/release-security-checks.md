...
`npm run test:coverage` inside `validate` drives the exact supported Pi 0.85.1 SDK/RPC fixtures. Evidence assertions parse their canonical formats: JSONL RPC records by event type/tool-call ID, mail journal JSON events by stable mail ID, registry JSON by exact address/generation, Pi session branches by entry/tool-call/message ID, process readiness/heartbeat JSON by schema, and the test runner's TAP result/exit status. Text grep counts are not release evidence.
...
Exact public `VERSION` admission plus the supplied ExtensionAPI facade check run before extension registration or broker/state construction and report actual/supported version or missing ExtensionAPI diagnostics. They do **not** certify event ordering, retries, durability, provider behavior, cleanup, or settings semantics. The exact tested Pi 0.85.1 dependency and the behavioral suite remain authoritative; wildcard host peers do not widen support. Clearly labelled Pi 0.84.2 observations elsewhere are historical characterizations, not a second supported host.

After writers stop changing the pushed candidate, choose the reviewed range base and run `npm exec -- tsx scripts/release-evidence.ts <review-base-commit> HEAD .test-workspaces/release-evidence`. The script fails before artifact generation unless candidate = HEAD = origin/main and the worktree/index are clean. Its parsed NUL-delimited inventories must agree, `git diff --check` must pass, and metadata must preserve those same facts, and `productionAudit` must record exit 0 for the included `production-audit.log` generated by the exact `npm audit --omit=dev --omit=peer` command. Then run the independent product-correctness and runtime-safety review gates against that exact commit. Do not release with an S0/S1 finding in enabled-by-default behavior.

Every release report must explicitly list untested coverage. Unless separately executed with preserved evidence, this includes paid/live providers, sudden power loss and filesystem-fsync durability, Windows namespace-owner/crash behavior, alternate Pi versions, and cross-parent workspace overlap. Deliberately detached OS processes are outside stop semantics rather than an untested containment promise. Deterministic custom-provider evidence is not a claim about every live provider.
