...
`npm run test:coverage` inside `validate` drives the pinned Pi 0.87.1 SDK/RPC fixtures. Evidence assertions parse their canonical formats: JSONL RPC records by event type/tool-call ID, mail journal JSON events by stable mail ID, registry JSON by exact address/generation, Pi session branches by entry/tool-call/message ID, process readiness/heartbeat JSON by schema, and the test runner's TAP result/exit status. Text grep counts are not release evidence.
...
The supplied ExtensionAPI facade check runs before registration or broker/state construction and reports missing APIs. There is no exact-version gate or upper Pi version bound. Capability checks do **not** certify event ordering, retries, durability, provider behavior, cleanup, or settings semantics. Pi 0.87.1 is the reproducible development baseline; the behavioral suite remains authoritative. Clearly labelled older Pi observations elsewhere are historical characterizations.
