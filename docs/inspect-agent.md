...
- When `cleanup` is present, do not interpret a detached worker or elapsed deadline as safety. Restart, archive, and clear-failure remain blocked. Only the separate `recover_cleanup` action can release an inactive settled unknown generation, and only after the human explicitly authorizes that exact address/generation with bounded external-quiescence evidence. Capacity pressure alone is never authorization. Its durable source is `operator-attested`, not Pi-verified, and it performs no restart, archive, cancellation, or mail delivery.
