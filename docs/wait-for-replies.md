...
| `collect` | boolean | `true` | Claim queued replies for this active wait when collection wins; ordinary-race losers return status without the already-presented body |
...
- Presentation ownership is one-directional and per wait. If ordinary presentation already won the serialized live race, this active wait returns terminal answered/status but omits that reply envelope/body. A fresh deliberate wait invoked later may still retrieve the answered reply, which preserves restart and uncertain-presentation recovery.
...
- High-priority steering stays immediate. A correlated high reply ordinarily presents even during an active collector; it ends a multi-ID wait partial promptly, and that wait omits the already-presented high body rather than waiting for a slow sibling or timeout.
...
The first line is `complete` (all terminal), `timed out with pending work` (`timedOut: true` with pending items), or `partial` (an abort or correlated high ordinary presentation ended the window with pending items and `timedOut: false`). With collection enabled, the next line states the active-race ownership rule, later-rejoin recovery, and Pi 0.84.2 crash limitation. Broker shutdown with pending work preserves the existing `timedOut: true` structured contract. Every `timedOut: true` result that still has pending items adds bounded guidance that the requests remain correlated, ordinary delivery is attempted when available, and an immediate keepalive-style rejoin is unnecessary. Complete and abort-partial results do not show that timeout guidance. `details.result` carries the same terminal metadata, but request/reply bodies are deliberately replaced with a marker instead of duplicating potentially large content already represented in the tool text.
