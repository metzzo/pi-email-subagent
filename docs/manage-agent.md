...
For the current blocked case, follow this exact sequence: **exit the live owner normally**; **resume the same session**; observe that broker **startup fails** closed; run `/agents recover-cleanup worker.p3-proposer@gpt-5.6-sol.com 9 --confirm Operator verified the exact external worker generation is quiescent`; run `/reload`; then explicitly `restart` or `archive` as appropriate. Never run the recovery while PID 15489 or its exact recorded identity remains live. A clone creates a fresh mailbox and does not recover old obligations.
