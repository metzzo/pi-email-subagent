...
While any child request or its reserved reply remains open, the parent parks: its run slot is released, upstream obligations remain unanswered, and mailbox enforcement does not consume attempts or spin. The parent remains responsible for every upstream request and cannot answer upstream while any child dependency is open. New upstream mail remains queued. An exact child reply is prioritized, wakes the persistent parent, and closes only the child request after accepted delivery; it never answers the parent's upstream request. Pi can begin the resumed parent turn immediately after prompt acceptance, so an upstream `send_email` tool call joins the exact in-flight child-answer commit and rechecks the dependency instead of racing it or failing spuriously. A failed parent keeps an accepted child reply queued under the same stable ID until explicit restart.
