...
For the current generation 9 case, never touch PID 15489 or its namespace while that exact recorded owner remains live. After it exits, resume the same parent session. Startup reclaims the exact dead-owner lock and performs the migration above. Then explicitly restart the same identity so its preserved session/mailbox can continue. A clone has a fresh mailbox and cannot recover the old obligations.
