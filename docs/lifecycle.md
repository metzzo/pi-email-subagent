...
Acquisition serializes owner inspection, stale-lock removal, filesystem locking, and new-owner publication with an owner-transition guard. If removing that guard fails after a new lease is published, initialization releases the lease, retries the transient removal, and fails with an operator-visible diagnostic instead of becoming active behind an abandoned guard. The caller namespace path must pass the same bounded/control-free predicate before any namespace, owner, lock, or transition artifact is created. Before any liveness comparison or removal, owner metadata requires that exact namespace, a canonical Linux boot-ID UUID, a positive canonical decimal start time, a positive safe PID, a nonempty bounded token, and a bounded canonical ISO timestamp. Generated metadata must round-trip through that validator, and release fails closed if its own exact owner is no longer recognizable. An exact live or `SIGSTOP`ed owner always rejects, regardless of lock mtime. When the complete exact recorded owner identity matches and that owner process is dead, startup automatically reclaims the stale proper-lockfile directory, acquires the namespace, and records `abandonedOwner: true`. Missing/incomplete/malformed identity, a lock without its sidecar, an owner-publication gap, a mismatched namespace, non-Linux exact-dead takeover, or an abandoned transition guard remains fail-closed without changing owner/lock bytes. On non-Linux hosts, an incomplete owner's PID may be reported as existing only to explain why a contender is blocked; PID existence does not establish exact-owner identity and never permits reclaim. An incomplete owner whose PID is absent or unknown remains unreclaimable. On Linux, a successful boot/start comparison mismatch establishes that the recorded exact generation is absent. A boot-ID or process-stat read failure alone never does: signal-0 success, `EPERM`, or any non-`ESRCH` error blocks as live/unverifiable, while only `ESRCH` confirms PID absence and may continue toward exact-dead recovery.
