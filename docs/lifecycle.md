...
The namespace lock is a cooperative per-parent state lease, not a workspace or security fence. On Linux, owner metadata records the boot ID and `/proc/<pid>/stat` start time. A stale mtime is never enough to take the lease from that exact live process, including a `SIGSTOP`ed owner. Takeover after exact owner death is surfaced as abandoned; before any writable restoration, prior mutation-capable restorable records become sticky unknown cleanup because descendants may have survived. Platforms without the required kernel identity fence allow clean ownership but fail closed on abandoned takeover rather than falling back to mtime-only recovery.
