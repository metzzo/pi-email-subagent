...
manage_agent(address, action, workerGeneration?, operatorEvidence?)
```
...
`send_email` returns the allocated request/correlation ID, exact expected reply subject, effective recipient role/tools/provider/model, finite persisted lifecycle policy, and delivery state. `inspect_agent` previews the same effective profile without spawning and reports derived identity-lease capacity separately from run concurrency. `wait_for_replies` opens a bounded collection window; after a pending timeout, requests remain correlated in durable mail, so no keepalive-style rejoin is needed. Pi 0.81.1 gives no durable native-session append acknowledgement for `sendMessage`, prompt preflight, `steer`, `followUp`, or a custom tool result. A crash can therefore leave journal and visible presentation state different. Rejoin the stable request ID or inspect Conversation/mail after restart; no ordinary-main or worker delivery is claimed exactly once. `cancel_request` durably closes one intentionally abandoned obligation to an inactive recipient, recording the actor and reason without fabricating a reply. `manage_agent` is main-thread-only; ordinary actions are `stop`, `restart`, `archive`, and `clear_failure`, while `recover_cleanup` is an exact-generation **proposal**. The model may propose it only after the user explicitly says they externally verified that exact generation's quiescence, supplying a bounded quote/summary as `operatorEvidence`. The tool always opens a one-use live Pi UI confirmation showing the canonical address, exact generation, sanitized evidence, and overlap warning before any online or startup-blocked offline recovery access. Model text alone has zero authority; denial, cancellation, timeout, no UI, and stale/replaced context fail closed. The literal `/agents recover-cleanup <exact-address> <worker-generation> --confirm <operator evidence>` remains a direct human alternative that a model must not invoke. A confirmed result is operator-attested, not Pi-verified; capacity pressure alone is never authorization. It releases only the exact settled inactive quarantine, remains durably `failed`, and never automatically restores/creates a worker, restarts, archives, delivers queued mail, or cancels obligations. Workers continue to receive only the two email tools. See the [safe capacity recovery order](docs/manage-agent.md#safe-identity-capacity-recovery).
...
- `/agents recover-cleanup <exact-address> <worker-generation> --confirm <operator evidence>` (direct human alternative; models use the confirmation-gated `manage_agent recover_cleanup` proposal)
...
State is stored under `~/.pi/agent/subagents/<parent-session-id>/`. A cooperative filesystem lease reduces accidental concurrent state writers only on the same local host and PID namespace; it is not a workspace, distributed-filesystem, container-boundary, or security fence. On Linux, boot ID plus kernel process-start identity prevents a stale mtime from taking the lease from an exact live or `SIGSTOP`ed owner. Missing, stale, or dead-generation sidecars/lock directories remain fail-closed orphans. Orphan manual recovery is exact and audited; elapsed stale time never authorizes automatic takeover. After an affirmative one-use Pi UI confirmation, `manage_agent recover_cleanup` and the direct-human literal `/agents recover-cleanup` share the same offline transition callback. It rechecks the recorded boot-ID plus `/proc/<pid>/stat` start identity, normalizes every other unsafe mutation-capable or conservative legacy writable record into abandoned-owner quarantine, backs up and atomically rewrites only the canonical registry, retains a generation-bound `operator-attested` audit, and removes only the exact dead owner/lock artifacts. Human evidence plus affirmative UI confirmation authorizes a model proposal; owner death does not prove quiescence, and the epoch is `operator-released`, not `verified-clean` or Pi-verified. A pre-existing `.cleanup-recovery.guard` is never automatically read/reclaimed/replaced: inspect its exact PID/boot-ID/process-start identity and the registry/backup crash stage, then only after proving that guard owner dead deliberately remove that one guard and retry.

For the blocked session `01a02af8-02af-74f2-a4a7-78828e71b8a9`, the safe sequence is: **exit the live owner normally** (do not run recovery while PID 15489 or its exact recorded identity is live); **resume the same session**; observe that broker **startup fails** closed on the orphan; after explicit human authorization run `/agents recover-cleanup worker.p3-proposer@gpt-5.6-sol.com 9 --confirm Operator verified the exact external worker generation is quiescent`; run `/reload`; then explicitly `restart` or `archive` the identity as appropriate. The command never reloads automatically, so failure evidence remains visible. A clone creates a fresh mailbox and does not recover old obligations from this parent session.
