...
Provider/catalog changes require extension reload. The current 0.1.0 candidate is tested on Pi 0.87.1 on Node 22.19.0 or newer; CI and the packed-artifact load smoke pin that development baseline. Earlier Pi behavior characterizations elsewhere remain historical evidence. Startup checks the supplied public ExtensionAPI facade before registration or broker/state construction, not an exact host version. Pi core packages remain host-provided wildcard peers with no upper version bound. This allows upgrades without blocking startup merely because the version changed; it does not guarantee compatibility with future API changes.
