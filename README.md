...
High-priority mail is offered to Pi's steer boundary for a running recipient. Low-priority worker mail waits until the recipient settles. Low mail to busy main stays in the bounded durable broker queue instead of becoming a Pi `followUp`: `wait_for_replies(collect:true)` can claim a correlated queued reply, otherwise it is offered at the next safe idle settlement boundary. A backlog may drain one per settlement; main-idle low mail presents promptly. Only queued low main mail consumes this bounded backlog; high main mail remains exempt and immediate. Only an exact successful `send_email` reply closes an obligation; visible final assistant text is never copied across mail IDs. A worker that settles with unanswered mail is prompted to send exact replies, and exhausted enforcement marks it failed without answering any request.
