...
Ordinary global/project extension discovery remains disabled in worker sessions. During main-session startup, pi-email-subagent synchronously emits `pi-email-subagent:collect-worker-extensions` with a protocol-v1 collector. An already loaded, trusted main extension may register `{ protocolVersion: 1, name, factory, tools }` to run that exact inline factory in every worker.

Registration names, counts, tool counts, and tool-name sizes are bounded. Built-in/mailbox names, duplicate registration names, duplicate/colliding tools, undeclared tools, and failed activation are rejected or reported with bounded diagnostics. Declared tools are added outside role profiles and remain conservatively effect-capable. Factories receive explicit Pi `print`-mode lifecycle binding; nested continuation settlements collapse to one worker-level settlement, and cleanup aborts compaction and joins the full admitted prompt before reporting quiescence.

`pi-compact-warning` uses this protocol so its warning steer, terminating `compact_and_continue` handoff, native compaction, and continuation steer also run inside email workers. Both extensions are trusted code with the Pi user's permissions; opt-in is not sandboxing.
