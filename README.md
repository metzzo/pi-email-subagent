...
State is stored under `~/.pi/agent/subagents/<parent-session-id>/`. A cooperative filesystem lease reduces accidental concurrent state writers only on the same local host and PID namespace; it is not a workspace, distributed-filesystem, container-boundary, or security fence. On Linux, boot ID plus kernel process-start identity prevents a stale mtime from taking the lease from an exact live or `SIGSTOP`ed owner. Missing, stale, or dead-generation sidecars/lock directories remain fail-closed orphans. Orphan manual recovery is exact and audited; elapsed stale time never authorizes automatic takeover. The explicit manual offline `/agents recover-cleanup` path rechecks the recorded boot-ID plus `/proc/<pid>/stat` start identity, normalizes every other unsafe mutation-capable or conservative legacy writable record into abandoned-owner quarantine, backs up and atomically rewrites only the canonical registry, retains a generation-bound `operator-attested` audit, and removes only the exact dead owner/lock artifacts. Human evidence authorizes quarantine release; owner death does not prove quiescence, and the epoch is `operator-released`, not `verified-clean` or Pi-verified. A pre-existing `.cleanup-recovery.guard` is never automatically read/reclaimed/replaced: inspect its exact PID/boot-ID/process-start identity and the registry/backup crash stage, then only after proving that guard owner dead deliberately remove that one guard and retry.
